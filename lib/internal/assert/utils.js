...
function getErrMessage(message, fn) {
  ...
  // The wasm32 interpreter does not materialize a native CallSite for every
  // emulated JSEntry frame. Keep assertion reporting on its normal fallback
  // path when no stack frame is available rather than throwing a second error.
  if (process.platform === 'wasi' &&
      (call == null || typeof call.getFileName !== 'function')) {
    return message;
  }
