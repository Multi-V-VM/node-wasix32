...
class ReadableStream {
  ...
  values(options = kEmptyObject) {
    ...
    return ObjectSetPrototypeOf({
      ...
      next() {
        // The wasm32 Promise fallback cannot reliably retain the pending
        // stream read through the PromisePrototypeThen serialization chain.
        // Async iteration already calls next() sequentially, so use the same
        // direct read path as ReadableStreamDefaultReader on WASI.
        if (process.platform === 'wasi') return reader.read();
