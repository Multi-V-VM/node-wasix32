...
// The WASM32 interpreter's first access to a lazy stream accessor can perform
// its initialization without propagating the accessor result to the caller.
// Prime both accessors twice: the first nested lazy access initializes its
// backing stream, and the second observes the initialized value.
...
function getStdout() {
  ...
  if (rawMethods._wasmWriteStdout) {
    stdout.write = function write(chunk, encoding, callback) {
      if (typeof encoding === 'function') {
        callback = encoding;
        encoding = undefined;
      }
      const text = typeof chunk === 'string' ? chunk : chunk.toString(encoding);
      rawMethods._wasmWriteStdout(text);
  ...
  if (stdout.isTTY && process.platform !== 'wasi') {
...
function getStderr() {
  ...
  if (rawMethods._wasmWriteStderr) {
    stderr.write = function write(chunk, encoding, callback) {
      if (typeof encoding === 'function') {
        callback = encoding;
        encoding = undefined;
      }
      const text = typeof chunk === 'string' ? chunk : chunk.toString(encoding);
      rawMethods._wasmWriteStderr(text);
      if (typeof callback === 'function') process.nextTick(callback);
      return true;
    };
  }
  ...
  if (stderr.isTTY && process.platform !== 'wasi') {
