...
class BuiltinModule {
  ...
  static exists(id) {
    return !!BuiltinModule.lookupCache[id] ||
           BuiltinModule.map.has(id) ||
           isBuiltin(id);
  }
...
for (let i = 0; i < builtinIds.length; ++i) {
  const id = builtinIds[i];
  const mod = BuiltinModule.create(id);
  BuiltinModule.lookupCache[id] = mod;
  BuiltinModule.map.set(id, mod);
}
