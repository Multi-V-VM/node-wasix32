...
class BuiltinModule {
  ...
  static lookupCache = { __proto__: null };
  static loadedCache = { __proto__: null };

  static get(id) {
    let mod = BuiltinModule.lookupCache[id];
    if (!mod) {
      mod = BuiltinModule.create(id);
      BuiltinModule.lookupCache[id] = mod;
    }
    return mod;
  }
  ...
  static create(id) {
    return {
      __proto__: BuiltinModule.prototype,
      filename: `${id}.js`,
      id,
      exports: {},
      loaded: false,
  ...
  static exists(id) {
    return !!BuiltinModule.lookupCache[id] || isBuiltin(id);
  }
  ...
  compileForInternalLoader() {
    const cachedExports = BuiltinModule.loadedCache[this.id];
    if (cachedExports) {
      this.exports = cachedExports;
      this.loaded = true;
      this.loading = false;
    ...
    try {
      ...
      this.loaded = true;
      BuiltinModule.loadedCache[id] = this.exports;
    } finally {
      this.loading = false;
    }
...
function requireWithFallbackInDeps(request) {
  ...
  } else if (!BuiltinModule.exists(request)) {
