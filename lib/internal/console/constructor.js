...
let wasmConsoleRawWrite;
function wasmConsoleWriteLine(fd, args) {
  wasmConsoleRawWrite ??= internalBinding('process_methods')._rawWrite;
  let line = '';
  for (let i = 0; i < args.length; ++i) {
    if (i !== 0) line += ' ';
    line += `${args[i]}`;
  }
  wasmConsoleRawWrite(fd, `${line}\n`);
}

function shouldUseWasmConsoleFastPath() {
  return true;
}
...
const consoleMethods = {
  log(...args) {
    ...
    if (shouldUseWasmConsoleFastPath()) {
      wasmConsoleWriteLine(1, args);
      return;
    }
