...
ChildProcess.prototype.spawn = function spawn(options) {
  ...
  // WASI cannot create child processes. Avoid entering the native spawn error
  // path, where no uv_process_t has been initialized to close.
  if (process.platform === 'wasi') {
    const err = new Error('spawn ENOSYS');
    err.code = 'ENOSYS';
    err.syscall = 'spawn';
    err.path = options.file;
    this._handle = null;
