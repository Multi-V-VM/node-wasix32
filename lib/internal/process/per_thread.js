...
let execveDiagnosticChannel;

function getExecveDiagnosticChannel() {
  execveDiagnosticChannel ??=
    require('diagnostics_channel').channel('process.execve');
  return execveDiagnosticChannel;
}
...
function wrapProcessMethods(binding) {
  ...
  function execve(execPath, args, env) {
    ...
    const channel = getExecveDiagnosticChannel();
    if (channel.hasSubscribers) {
      channel.publish({ execPath, args, env: envArray });
    }
...
function buildAllowedFlags() {
  ...
  // The WASI embedder can omit CLI option metadata. Fail closed instead of
  // attempting to iterate an absent native binding result.
  if (options == null || aliases == null) return ObjectFreeze(new Set());
