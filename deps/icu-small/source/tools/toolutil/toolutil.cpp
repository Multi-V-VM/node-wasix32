...
void setCPTrieBit(UMutableCPTrie *mutableCPTrie, UChar32 start, UChar32 end, int32_t shift, bool on,
...
void setCPTrieBits(UMutableCPTrie *mutableCPTrie, UChar32 start, UChar32 end, uint32_t mask,
                   uint32_t value, UErrorCode &errorCode) {
    ...
    while (start <= end && U_SUCCESS(errorCode)) {
        uint32_t oldValue;
        UChar32 rangeEnd = umutablecptrie_getRange(mutableCPTrie, start, UCPMAP_RANGE_NORMAL, 0, nullptr,
                                                   nullptr, &oldValue);
        if (rangeEnd > end) {
            rangeEnd = end;
        }
...
int32_t getCPTrieSize(UMutableCPTrie *mt, UCPTrieType type, UCPTrieValueWidth valueWidth) {
    ...
    if (U_FAILURE(errorCode)) {
        fprintf(stderr, "toolutil/getCPTrieSize error: umutablecptrie_buildImmutable() failed: %s\n",
    ...
    if (U_FAILURE(errorCode) && errorCode != U_BUFFER_OVERFLOW_ERROR) {
        fprintf(stderr, "toolutil/getCPTrieSize error: ucptrie_toBinary() failed: %s (length %ld)\n",
    ...
    if (file != INVALID_HANDLE_VALUE) {
        if (info.cAlternateFileName[0] != 0) {
            ...
            if (basename != pathname) {
                ...
                uprv_memmove(info.cFileName + (basename - pathname), info.cFileName,
...
U_CAPI const char *U_EXPORT2 findDirname(const char *path, char *buffer, int32_t bufLen,
                                         UErrorCode *status) {
...
#if U_FILE_ALT_SEP_CHAR != U_FILE_SEP_CHAR
    // be lenient about pathname separators on Windows, like official implementation of C++17
    // std::filesystem in MSVC would be convenient to merge this loop with the one above, but alas, there
    // is no such solution in the standard library
...
U_CAPI UToolMemory *U_EXPORT2 utm_open(const char *name, int32_t initialCapacity, int32_t maxCapacity,
                                       int32_t size) {
    UToolMemory *mem;
...
U_CAPI void *U_EXPORT2 utm_getStart(UToolMemory *mem) { return (char *)mem->array; }

U_CAPI int32_t U_EXPORT2 utm_countItems(UToolMemory *mem) { return mem->idx; }

static UBool utm_hasCapacity(UToolMemory *mem, int32_t capacity) {
    if (mem->capacity < capacity) {
        int32_t newCapacity;

        if (mem->maxCapacity < capacity) {
            fprintf(stderr, "error: %s - trying to use more than maxCapacity=%ld units\n", mem->name,
                    static_cast<long>(mem->maxCapacity));
            exit(U_MEMORY_ALLOCATION_ERROR);
        }
