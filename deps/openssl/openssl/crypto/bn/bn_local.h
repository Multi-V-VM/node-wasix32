...
#define bn_pollute(a)                                                          \
  do {                                                                         \
    ...
    if (_bnum1->top < _bnum1->dmax) {                                          \
      ...
      (void)RAND_bytes(&_tmp_char,                                             \
                       1); /* Debug only - safe to ignore error return */      \
      memset(_not_const + _bnum1->top,                                         \
             _tmp_char,                                                        \
...
#define BN_UMULT_HIGH(a, b)                                                    \
  ({                                                                           \
    register BN_ULONG ret;                                                     \
    asm("dmultu    %1,%2" : "=h"(ret) : "r"(a), "r"(b) : "l");                 \
    ret;                                                                       \
  })
#define BN_UMULT_LOHI(low, high, a, b)                                         \
  asm("dmultu    %2,%3" : "=l"(low), "=h"(high) : "r"(a), "r"(b));
#endif
...
void bn_mul_low_recursive(
    BN_ULONG* r, BN_ULONG* a, BN_ULONG* b, int n2, BN_ULONG* t);
BN_ULONG bn_sub_part_words(
    BN_ULONG* r, const BN_ULONG* a, const BN_ULONG* b, int cl, int dl);
...
int ossl_bn_check_prime(const BIGNUM* w,
                        int checks,
                        BN_CTX* ctx,
