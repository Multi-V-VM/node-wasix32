...
wasm::AsmJsParser::VarInfo* AsmJsParser::GetVarInfo(
  ...
  if (index + 1 > old_capacity) {
    ...
    ZoneVector<VarInfo> new_info = zone_->NewZoneVector<VarInfo>(new_size);
...
void AsmJsParser::AddGlobalImport(ZoneVector<const char> name, AsmType* type,
...
void AsmJsParser::ValidateModuleVarImport(VarInfo* info,
  ...
  if (Check('+')) {
    ...
    EXPECT_TOKEN('.');
    ZoneVector<const char> name = CopyCurrentIdentifierString();
...
void AsmJsParser::ValidateFunction() {
  ...
  ZoneVector<const char> function_name_str = CopyCurrentIdentifierString();
