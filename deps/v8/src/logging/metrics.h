...
template <class T, int64_t (::v8::base::TimeDelta::*precision)() const =
                       &::v8::base::TimeDelta::InMicroseconds>
class V8_NODISCARD TimedScope {
 public:
  explicit TimedScope(T* event) : event_(event) { Start(); }
  ~TimedScope() { Stop(); }

  void Start() { start_time_ = ::v8::base::TimeTicks::Now(); }
  ...
  void Stop() {
    ...
    start_time_ = ::v8::base::TimeTicks::Min();
  }
