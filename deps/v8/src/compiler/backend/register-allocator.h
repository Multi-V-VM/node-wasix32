...
class DoubleEndedSplitVector {
 public:
  ...
  DoubleEndedSplitZoneVector<T> SplitAt(const_iterator split_begin_const) {
  ...
  // allocate or copy.
  void Append(Zone* zone, DoubleEndedSplitZoneVector<T> other) {
    ...
    // this path anyway.
    DoubleEndedSplitZoneVector<T> result;
 ...
 private:
  ...
  V8_NOINLINE V8_PRESERVE_MOST void GrowAt(Zone* zone,
    ...
    DoubleEndedSplitZoneVector<T> old = std::move(*this);
...
using UseIntervalVector = DoubleEndedSplitZoneVector<UseInterval>;
using UsePositionVector = DoubleEndedSplitZoneVector<UsePosition*>;
...
class V8_EXPORT_PRIVATE LiveRange : public NON_EXPORTED_BASE(ZoneObject) {
 public:
  ...
  ZoneVector<UsePosition*> positions() const { return positions_span_; }
 ...
 private:
  ...
  ZoneVector<UsePosition*> positions_span_;
...
class LinearScanAllocator final : public RegisterAllocator {
 ...
 private:
  ...
  bool TryAllocatePreferredReg(
      LiveRange* range, ZoneVector<const LifetimePosition> free_until_pos);
  ...
  void FindFreeRegistersForRange(LiveRange* range,
                                 ZoneVector<LifetimePosition> free_until_pos);
