...
void TransitionsAccessor::InsertHelper(Isolate* isolate, DirectHandle<Map> map,
...

  {
    ...
    if (index != kNotFound) {
      ::v8::base::MutexGuard mutex_guard(isolate->full_transition_array_access());
...
Tagged<Map> TransitionsAccessor::SearchTransition(
  ...
  switch (encoding()) {
    ...
    case kFullTransitionArray: {
      ::v8::base::MutexGuardIf guard(isolate_->full_transition_array_access(),
                               concurrent_access_);
...
bool TransitionsAccessor::PutPrototypeTransition(Isolate* isolate,
  ...
  if (transitions > capacity) {
    ...
    bool compacted;
    {
      ::v8::base::MutexGuard guard(isolate->full_transition_array_access());
      DisallowGarbageCollection no_gc;
      compacted =
