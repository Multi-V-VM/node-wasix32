...
bool String::MakeExternal(Isolate* isolate,
  ...
  if (v8_flags.enable_slow_asserts) {
    ...
    base::ScopedZoneVector<base::uc16> smart_chars(this->length());
...
bool String::MakeExternal(Isolate* isolate,
  ...
  if (v8_flags.enable_slow_asserts) {
    ...
    if (this->IsTwoByteRepresentation()) {
      base::ScopedZoneVector<uint16_t> smart_chars(this->length());
      String::WriteToFlat(this, smart_chars.begin(), 0, this->length());
      DCHECK(String::IsOneByte(smart_chars.begin(), this->length()));
    }
    base::ScopedZoneVector<char> smart_chars(this->length());
    String::WriteToFlat(this, smart_chars.begin(), 0, this->length());
    DCHECK_EQ(0, memcmp(smart_chars.begin(), resource->data(),
                        resource->length() * sizeof(smart_chars[0])));
  }
...
static void CalculateLineEndsImpl(String::LineEndsVector* line_ends,
                                  ZoneVector<const SourceChar> src,
...
ComparisonResult String::Compare(Isolate* isolate, DirectHandle<String> x,
  ...
  if (x_content.IsOneByte()) {
    ZoneVector<const uint8_t> x_chars = x_content.ToOneByteVector();
    if (y_content.IsOneByte()) {
      ZoneVector<const uint8_t> y_chars = y_content.ToOneByteVector();
      r = CompareChars(x_chars.begin(), y_chars.begin(), prefix_length);
    } else {
      ZoneVector<const base::uc16> y_chars = y_content.ToUC16Vector();
      r = CompareChars(x_chars.begin(), y_chars.begin(), prefix_length);
    }
  } else {
    ZoneVector<const base::uc16> x_chars = x_content.ToUC16Vector();
    if (y_content.IsOneByte()) {
      ZoneVector<const uint8_t> y_chars = y_content.ToOneByteVector();
      r = CompareChars(x_chars.begin(), y_chars.begin(), prefix_length);
    } else {
...
int SearchString(Isolate* isolate, String::FlatContent receiver_content,
                 ZoneVector<T> pat_vector, int start_index) {
...
int String::IndexOf(Isolate* isolate, DirectHandle<String> receiver,
  ...
  if (search_content.IsOneByte()) {
    ZoneVector<const uint8_t> pat_vector = search_content.ToOneByteVector();
    return SearchString<const uint8_t>(isolate, receiver_content, pat_vector,
                                       start_index);
  }
  ZoneVector<const base::uc16> pat_vector = search_content.ToUC16Vector();
...
int StringMatchBackwards(ZoneVector<const schar> subject,
                         ZoneVector<const pchar> pattern, int idx) {
...
bool String::HasOneBytePrefix(ZoneVector<const char> str) {
  DCHECK(!SharedStringAccessGuardIfNeeded::IsNeeded(this));
...
bool IsIdentifierVector(ZoneVector<Char> vec) {
  if (vec.empty()) {
    return false;
  }
