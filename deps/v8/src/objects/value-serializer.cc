...
void ValueSerializer::WriteOneByteString(::v8::base::Vector<const uint8_t> chars) {
  WriteVarint<uint32_t>(chars.length());
  WriteRawBytes(chars.begin(), chars.length() * sizeof(uint8_t));
}

void ValueSerializer::WriteTwoByteString(::v8::base::Vector<const base::uc16> chars) {
  // Warning: this uses host endianness.
...
void ValueSerializer::WriteString(DirectHandle<String> string) {
  ...
  DCHECK(flat.IsFlat());
  if (flat.IsOneByte()) {
    ::v8::base::Vector<const uint8_t> chars = flat.ToOneByteVector();
    WriteTag(SerializationTag::kOneByteString);
    WriteOneByteString(chars);
  } else if (flat.IsTwoByte()) {
    ::v8::base::Vector<const base::uc16> chars = flat.ToUC16Vector();
...
Maybe<::v8::base::Vector<const uint8_t>> ValueDeserializer::ReadRawBytes(
    size_t size) {
  if (size > static_cast<size_t>(end_ - position_)) {
    return Nothing<::v8::base::Vector<const uint8_t>>();
  }
  const uint8_t* start = position_;
  position_ += size;
  return Just(::v8::base::Vector<const uint8_t>(start, size));
}

Maybe<::v8::base::Vector<const base::uc16>> ValueDeserializer::ReadRawTwoBytes(
    size_t size) {
  if (size > static_cast<size_t>(end_ - position_) ||
      size % sizeof(base::uc16) != 0) {
    return Nothing<::v8::base::Vector<const base::uc16>>();
  }
  const base::uc16* start = (const base::uc16*)(position_);
  position_ += size;
  return Just(::v8::base::Vector<const base::uc16>(start, size / sizeof(base::uc16)));
}
...
MaybeDirectHandle<String> ValueDeserializer::ReadUtf8String(
  ...
  ::v8::base::Vector<const uint8_t> utf8_bytes;
  if (!ReadRawBytes(utf8_length).To(&utf8_bytes)) return {};
  return isolate_->factory()->NewStringFromUtf8(
      ::v8::base::Vector<const char>::cast(utf8_bytes), allocation);
}
...
static void CommitProperties(
    ...
    ::v8::base::Vector<const DirectHandle<Object>> properties) {
...
Maybe<uint32_t> ValueDeserializer::ReadJSObjectProperties(
  ...
  if (can_use_transitions) {
    ...
    DirectHandle<::v8::base::Vector<Object> properties(isolate_);
    ...
    while (transitioning) {
      ...
      std::pair<DirectHandle<String>, DirectHandle<Map>> expected_transition;
      {
        ...
        if (tag == SerializationTag::kOneByteString) {
          ::v8::base::Vector<const uint8_t> key_chars;
          ...
          ::v8::base::Vector<const base::uc16> key_chars;
...
ValueDeserializer::ReadObjectUsingEntireBufferForLegacyFormat() {
  ...
  HandleScope scope(isolate_);
  DirectHandle<::v8::base::Vector<Object> stack(isolate_);
