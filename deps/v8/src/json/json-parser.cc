...
class JSDataObjectBuilder {
 public:
  ...
  V8_INLINE bool TryAddFastPropertyForValue(ZoneVector<const Char> key_chars,
 ...
 private:
  ...
  V8_INLINE bool TryFastTransitionToPropertyKey(
      ZoneVector<const Char> key_chars, GetKeyFunction&& get_key,
...
class JsonParser<Char>::NamedPropertyIterator {
 public:
  ...
  ZoneVector<const Char> GetKeyChars() {
...
bool JsonParser<Char>::ParseJsonNumberAsDoubleOrSmi(double* result_double,
...

  {
    ...
    ZoneVector<const Char> chars(start, cursor_ - start);
    *result_double =
...
template <typename Char>
bool Matches(ZoneVector<const Char> chars, DirectHandle<String> string) {
...
Handle<String> JsonParser<Char>::DecodeString(
  ...
  using SinkChar = typename SinkSeqString::Char;
  {
    ...
    ZoneVector<const SinkChar> data(dest, string.length());
...
Handle<String> JsonParser<Char>::MakeString(const JsonString& string,
  ...
  if (string.internalize() && !string.has_escape()) {
    if (!hint.is_null()) {
      ZoneVector<const Char> data(chars_ + string.start(), string.length());
    ...
    ZoneVector<const Char> chars(chars_ + string.start(), string.length());
