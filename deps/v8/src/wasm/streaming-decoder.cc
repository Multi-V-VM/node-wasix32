 ...
 private:
  ...
  class SectionBuffer : public WireBytesStorage {
   public:
    ...
    SectionBuffer(uint32_t module_offset, uint8_t id, size_t payload_length,
                  ZoneVector<const uint8_t> length_bytes)
        :  // ID + length + payload
          module_offset_(module_offset),
          bytes_(base::OwnedZoneVector<uint8_t>::NewForOverwrite(
    ...
    ZoneVector<uint8_t> bytes() const { return bytes_.as_vector(); }
    ZoneVector<uint8_t> payload() const { return bytes() + payload_offset_; }
    size_t length() const { return bytes_.size(); }
    size_t payload_offset() const { return payload_offset_; }

   private:
    const uint32_t module_offset_;
    const base::OwnedZoneVector<uint8_t> bytes_;
  ...
  class DecodingState {
   public:
    ...
    virtual ZoneVector<uint8_t> buffer() = 0;
  ...
  void ProcessFunctionBody(ZoneVector<const uint8_t> bytes,
...
void AsyncStreamingDecoder::OnBytesReceived(ZoneVector<const uint8_t> bytes) {
...
size_t AsyncStreamingDecoder::DecodingState::ReadBytes(
    AsyncStreamingDecoder* streaming, ZoneVector<const uint8_t> bytes) {
  ZoneVector<uint8_t> remaining_buf = buffer() + offset();
...
void AsyncStreamingDecoder::Finish(bool can_use_compiled_module) {
  ...
  base::OwnedZoneVector<const uint8_t> bytes_copy;
  ...
  if (!full_wire_bytes_.back().empty()) {
    ...
    auto all_bytes = base::OwnedZoneVector<uint8_t>::NewForOverwrite(total_length);
...
class AsyncStreamingDecoder::DecodeSectionID : public DecodingState {
 public:
  ...
  ZoneVector<uint8_t> buffer() override { return {&id_, 1}; }
...
class AsyncStreamingDecoder::DecodeSectionPayload : public DecodingState {
 public:
  ...
  ZoneVector<uint8_t> buffer() override { return section_buffer_->payload(); }
...
class AsyncStreamingDecoder::DecodeFunctionBody : public DecodingState {
 public:
  ...
  ZoneVector<uint8_t> buffer() override {
    ZoneVector<uint8_t> remaining_buffer =
...
size_t AsyncStreamingDecoder::DecodeVarInt32::ReadBytes(
    AsyncStreamingDecoder* streaming, ZoneVector<const uint8_t> bytes) {
  ZoneVector<uint8_t> buf = buffer();
  ZoneVector<uint8_t> remaining_buf = buf + offset();
...
AsyncStreamingDecoder::DecodeNumberOfFunctions::NextWithValue(
  ...
  ZoneVector<uint8_t> payload_buf = section_buffer_->payload();
...
AsyncStreamingDecoder::DecodeFunctionLength::NextWithValue(
  ...
  ZoneVector<uint8_t> fun_length_buffer =
      section_buffer_->bytes() + buffer_offset_;
