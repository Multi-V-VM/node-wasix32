...
class TypeCanonicalizer {
 ...
 private:
  ...
  struct CanonicalEquality {
    ...
    bool EqualTypes(ZoneVector<const CanonicalType> types1,
                    ZoneVector<const CanonicalType> types2) const {
  ...
  struct CanonicalGroup {
    CanonicalGroup(Zone* zone, size_t size, CanonicalTypeIndex first)
        : types(zone->AllocateZoneVector<CanonicalType>(size)), first(first) {
    ...
    bool operator==(const CanonicalGroup& other) const {
      ...
      return equality.EqualTypes(ZoneVector<const CanonicalType>(types.begin(),
                                                                types.size()),
                                 ZoneVector<const CanonicalType>(
                                     other.types.begin(), other.types.size()));
    }
