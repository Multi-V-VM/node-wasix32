...
FunctionSig* CreateSignature(Zone* zone,
                             ZoneVector<const ValueType> param_types,
                             ZoneVector<const ValueType> return_types) {
...
class GeneratorAlternativesPerOption {
 ...
 public:
  ...
  constexpr ZoneVector<const GenerateFn> GetAlternatives(
...
class BodyGen {
  ...
  class V8_NODISCARD BlockScope {
   public:
    BlockScope(BodyGen* gen, WasmOpcode block_type,
               ZoneVector<const ValueType> param_types,
               ZoneVector<const ValueType> result_types,
               ZoneVector<const ValueType> br_types, bool emit_end = true)
  ...
  void block(ZoneVector<const ValueType> param_types,
             ZoneVector<const ValueType> return_types, DataRange* data) {
  ...
  void loop(ZoneVector<const ValueType> param_types,
            ZoneVector<const ValueType> return_types, DataRange* data) {
  ...
  void finite_loop(ZoneVector<const ValueType> param_types,
                   ZoneVector<const ValueType> return_types,
                   DataRange* data) {
  ...
  void if_(ZoneVector<const ValueType> param_types,
           ZoneVector<const ValueType> return_types, IfType type,
           DataRange* data) {
  ...
  void try_block_helper(ValueType return_type, DataRange* data) {
    ...
    ZoneVector<const ValueType> return_type_vec =
        return_type.kind() == kVoid ? ZoneVector<ValueType>{}
  ...
  void try_table_rec(ZoneVector<const ValueType> param_types,
                     ZoneVector<const ValueType> return_types,
                     ZoneVector<CatchCase> catch_cases, size_t i,
                     DataRange* data) {
    ...
    auto block_returns =
        builder_->builder()->zone()->AllocateZoneVector<ValueType>(return_count);
    if (has_tag) {
  ...
  void try_table_block_helper(ZoneVector<const ValueType> param_types,
                              ZoneVector<const ValueType> return_types,
                              DataRange* data) {
    uint8_t num_catch = data->get<uint8_t>() % kMaxCatchCases;
    auto catch_cases =
        builder_->builder()->zone()->AllocateZoneVector<CatchCase>(num_catch);
    for (int i = 0; i < num_catch; ++i) {
  ...
  void any_block(ZoneVector<const ValueType> param_types,
                 ZoneVector<const ValueType> return_types, DataRange* data) {
 ...
 public:
  ...
  void Generate(ZoneVector<const ValueType> types, DataRange* data) {
    ...
    ZoneVector<const ValueType> lower_half = types.SubVector(0, split_index);
    ZoneVector<const ValueType> upper_half =
  ...
  void Generate(std::initializer_list<ValueTypeBase> types, DataRange* data) {
    ZoneVector<const ValueType> cast_types = base::VectorOf<const ValueType>(
  ...
  void ConsumeAndGenerate(ZoneVector<const ValueType> param_types,
                          ZoneVector<const ValueType> return_types,
                          DataRange* data) {
...
class ModuleGen {
 public:
  ...
  StringImports AddImportedStringImports() {
    ...
    static constexpr ZoneVector<const char> kJsString =
        base::StaticCharVector("wasm:js-string");
    static constexpr ZoneVector<const char> kTextDecoder =
        base::StaticCharVector("wasm:text-decoder");
    static constexpr ZoneVector<const char> kTextEncoder =
...
Vector<uint8_t> GenerateRandomWasmModule(
  ...
  for (int i = 0; i < num_functions; ++i) {
    ...
    ZoneVector<const ValueType> return_types(sig->returns().begin(),
                                               sig->return_count());
...
Vector<uint8_t> GenerateWasmModuleForInitExpressions(
    Zone* zone, ZoneVector<const uint8_t> data, size_t* count) {
  ...
  *count = num_functions;
  ...
  for (int i = 0; i < num_globals; ++i) {
    ...
    auto buffer = zone->AllocateZoneVector<char>(8);
...
void EmitDeoptAndReturnValues(BodyGen gen_body, WasmFunctionBuilder* f,
  ...
  ZoneVector<const ValueType> return_types = f->signature()->returns();
...
Vector<uint8_t> GenerateWasmModuleForDeopt(
    Zone* zone, ZoneVector<const uint8_t> data,
  ...
  for (int i = 0; i < num_inlinees; ++i) {
    ...
    f->Emit(kExprEnd);
    auto buffer = zone->AllocateZoneVector<char>(32);
  ...
  for (int i = 0; i < num_call_targets; ++i) {
    ...
    BodyGen gen_body(options, f, function_signatures, {}, {}, struct_types,
                     array_types, strings, &function_range);
    const FunctionSig* sig = f->signature();
    ZoneVector<const ValueType> target_return_types(sig->returns().begin(),
                                                      sig->return_count());
    gen_body.InitializeNonDefaultableLocals(&function_range);
