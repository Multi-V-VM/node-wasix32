...
void ProcessPerFunctionCompileHints(const ZoneVector<const uint8_t>& data,
...
void Scanner::TryToParseMagicComment(base::uc32 hash_or_at_sign) {
  ...
  ZoneVector<const uint8_t> name_literal = name.one_byte_literal();
  LiteralBuffer* value;
  ...
  if (value == &per_function_compile_hints_value &&
    ...
    ZoneVector<const uint8_t> value_literal =
...
Token::Value Scanner::ScanIdentifierOrKeywordInnerSlow(bool escaped,
  ...
  if (can_be_keyword && next().literal_chars.is_one_byte()) {
    ZoneVector<const uint8_t> chars = next().literal_chars.one_byte_literal();
    Token::Value token =
...
const char* Scanner::CurrentLiteralAsCString(Zone* zone) const {
  DCHECK(is_literal_one_byte());
  ZoneVector<const uint8_t> vector = literal_one_byte_string();
