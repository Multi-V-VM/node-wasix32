...
bool SourceCodeCache::Lookup(Isolate* isolate, ZoneVector<const char> name,
...
void SourceCodeCache::Add(Isolate* isolate, ZoneVector<const char> name,
  ...
  DirectHandle<String> str =
      factory
          ->NewStringFromOneByte(ZoneVector<const uint8_t>::cast(name),
                                 AllocationType::kOld)
          .ToHandleChecked();
  DCHECK(!str.is_null());
...
bool Genesis::CompileExtension(Isolate* isolate, v8::Extension* extension) {
  ...
  ZoneVector<const char> name = base::CStrVector(extension->name());
