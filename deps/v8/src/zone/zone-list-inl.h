...
template <typename T>
void ZoneList<T>::AddAll(ZoneVector<const T> other, Zone* zone) {
...
template <typename T>
ZoneVector<T> ZoneList<T>::AddBlock(T value, int count, Zone* zone) {
  int start = length_;
  for (int i = 0; i < count; i++) Add(value, zone);
  return ZoneVector<T>(&data_[start], count);
}
