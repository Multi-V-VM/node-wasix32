...
class ZoneList final : public ZoneObject {
 public:
  ...
  ZoneList(ZoneVector<const T> other, Zone* zone)
      : ZoneList(other.length(), zone) {
    AddAll(other, zone);
  }
  ...
  ZoneVector<T> ToVector() const { return ZoneVector<T>(data_, length_); }
  ZoneVector<T> ToVector(int start, int length) const {
    DCHECK_LE(start, length_);
    return ZoneVector<T>(&data_[start], std::min(length_ - start, length));
  }

  ZoneVector<const T> ToConstVector() const {
    return ZoneVector<const T>(data_, length_);
  }
  ...
  void AddAll(ZoneVector<const T> other, Zone* zone);
  ...
  ZoneVector<T> AddBlock(T value, int count, Zone* zone);
