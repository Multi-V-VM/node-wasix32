...
class SimpleStringBuilder final {
 public:
  ...
  explicit SimpleStringBuilder(size_t size) {
    buffer_ = ZoneVector<char>::New(size);
...
class StringToIntHelper {
 ...
 protected:
  ...
  ZoneVector<const uint8_t> GetOneByteVector(
      const DisallowGarbageCollection& no_gc) {
    if (raw_one_byte_subject_ != nullptr) {
      return ZoneVector<const uint8_t>(raw_one_byte_subject_, length_);
    }
    return subject_->GetFlatContent(no_gc).ToOneByteVector();
  }

  ZoneVector<const base::uc16> GetTwoByteVector(
      const DisallowGarbageCollection& no_gc) {
    if (raw_two_byte_subject_ != nullptr) {
      return ZoneVector<const base::uc16>(raw_two_byte_subject_, length_);
    }
...
void StringToIntHelper::ParseInt() {
  ...
  if (IsOneByte()) {
    ZoneVector<const uint8_t> vector = GetOneByteVector(no_gc);
    DetectRadixInternal(vector.begin(), vector.size());
    if (state_ != State::kRunning) return;
    ParseOneByte(vector.begin());
  } else {
    ZoneVector<const base::uc16> vector = GetTwoByteVector(no_gc);
    DetectRadixInternal(vector.begin(), vector.size());
    if (state_ != State::kRunning) return;
...
class NumberParseIntHelper : public StringToIntHelper {
 ...
 private:
  ...
  void HandleBaseTenCase(const Char* current, const Char* end) {
    ...
    buffer[buffer_pos] = '\0';
    ZoneVector<const char> buffer_vector(buffer, buffer_pos);
...
double StringToDouble(ZoneVector<const uint8_t> str, ConversionFlag flags,
                      double empty_string_val) {
  return InternalStringToDouble(str.begin(), str.end(), flags,
                                empty_string_val);
}

double StringToDouble(ZoneVector<const base::uc16> str, ConversionFlag flags,
                      double empty_string_val) {
  return InternalStringToDouble(str.begin(), str.end(), flags,
                                empty_string_val);
}

double BinaryStringToDouble(ZoneVector<const uint8_t> str) {
  DCHECK_EQ(str[0], '0');
  DCHECK_EQ(tolower(str[1]), 'b');
  return InternalStringToIntDouble<1>(str.begin() + 2, str.end(), false, false);
}

double OctalStringToDouble(ZoneVector<const uint8_t> str) {
  DCHECK_EQ(str[0], '0');
  DCHECK_EQ(tolower(str[1]), 'o');
  return InternalStringToIntDouble<3>(str.begin() + 2, str.end(), false, false);
}

double HexStringToDouble(ZoneVector<const uint8_t> str) {
  DCHECK_EQ(str[0], '0');
  DCHECK_EQ(tolower(str[1]), 'x');
  return InternalStringToIntDouble<4>(str.begin() + 2, str.end(), false, false);
}

double ImplicitOctalStringToDouble(ZoneVector<const uint8_t> str) {
...
std::unique_ptr<char[]> BigIntLiteralToDecimal(
    LocalIsolate* isolate, ZoneVector<const uint8_t> literal) {
  StringToBigIntHelper<LocalIsolate> helper(nullptr, literal.begin(),
                                            literal.size());
  return helper.DecimalString(isolate->bigint_processor());
}

std::string_view DoubleToStringView(double v, ZoneVector<char> buffer) {
  switch (FPCLASSIFY_NAMESPACE::fpclassify(v)) {
    ...
    default: {
      ...
      base::DoubleToAscii(
          ...
          ZoneVector<char>(decimal_rep, kV8DtoaBufferCapacity), &sign,
...
std::string_view IntToStringView(int n, ZoneVector<char> buffer) {
  bool negative = true;
  if (n >= 0) {
    n = -n;
    negative = false;
...
std::string_view DoubleToFixedStringView(double value, int f,
  ...
  base::DoubleToAscii(value, base::DTOA_FIXED, f,
                      ZoneVector<char>(decimal_rep, kDecimalRepCapacity),
  ...
  ZoneVector<char> rep_buffer = ZoneVector<char>::New(rep_length + 1);
...
static std::string_view CreateExponentialRepresentation(
    ...
    int significant_digits, ZoneVector<char> buffer) {
...
std::string_view DoubleToExponentialStringView(double value, int f,
  ...
  if (f == -1) {
    base::DoubleToAscii(value, base::DTOA_SHORTEST, 0,
                        ZoneVector<char>(decimal_rep, kV8DtoaBufferCapacity),
                        &sign, &decimal_rep_length, &decimal_point);
...
std::optional<double> TryStringToDouble(LocalIsolate* isolate,
  ...
  ZoneVector<const base::uc16> v(buffer.get(), length);
...
bool IsSpecialIndex(Tagged<String> string,
  ...
  ZoneVector<const uint16_t> vector(buffer, length);
  ...
  ZoneVector<char> reverse_vector(reverse_buffer, arraysize(reverse_buffer));
