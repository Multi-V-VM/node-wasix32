...
class Float32 {
 public:
  ...
  explicit Float32(float value)
      : bit_pattern_(v8::base::bit_cast<uint32_t>(value)) {
  ...
  float get_scalar() const { return v8::base::bit_cast<float>(bit_pattern_); }
...
class Float64 {
 public:
  ...
  explicit Float64(double value)
      : bit_pattern_(v8::base::bit_cast<uint64_t>(value)) {
    // Check that the provided value is not a NaN, because the bit pattern of a
    // NaN may be changed by a base::bit_cast, e.g. for signalling NaNs on
    // ia32.
    DCHECK(!std::isnan(value));
  ...
  explicit Float64(v8::base::Double value) : bit_pattern_(value.AsUint64()) {}

  uint64_t get_bits() const { return bit_pattern_; }
  double get_scalar() const { return v8::base::bit_cast<double>(bit_pattern_); }
