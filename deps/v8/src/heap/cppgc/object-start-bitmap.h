...
HeapObjectHeader* ObjectStartBitmap::FindHeader(
  ...
  const int leading_zeroes = ::v8::base::bits::CountLeadingZeros(static_cast<uint8_t>(byte));
  object_start_number =
...
inline void ObjectStartBitmap::Iterate(Callback callback) const {
  ...
  for (size_t cell_index = 0; cell_index < kReservedForBitmap; cell_index++) {
    ...
    while (value) {
      const int trailing_zeroes = ::v8::base::bits::CountTrailingZeros(static_cast<uint64_t>(value));
