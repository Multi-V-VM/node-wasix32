...
Handle<String> FactoryBase<Impl>::MakeOrFindTwoCharacterString(uint16_t c1,
  ...
  if ((c1 | c2) <= unibrow::Latin1::kMaxChar) {
    ...
    return InternalizeString(ZoneVector<const uint8_t>(buffer, 2));
  }
  uint16_t buffer[] = {c1, c2};
  return InternalizeString(ZoneVector<const uint16_t>(buffer, 2));
}
...
Handle<String> FactoryBase<Impl>::InternalizeString(
    ZoneVector<const uint8_t> string, bool convert_encoding) {
...
template <typename Impl>
Handle<String> FactoryBase<Impl>::InternalizeString(
    ZoneVector<const uint16_t> string, bool convert_encoding) {
...
Handle<SeqTwoByteString> FactoryBase<Impl>::NewTwoByteInternalizedString(
    ZoneVector<const base::uc16> str, uint32_t raw_hash_field) {
...
Handle<String> FactoryBase<Impl>::LookupSingleCharacterStringFromCode(
  ...
  return InternalizeString(ZoneVector<const uint16_t>(buffer, 1));
}

template <typename Impl>
MaybeHandle<String> FactoryBase<Impl>::NewStringFromOneByte(
    ZoneVector<const uint8_t> string, AllocationType allocation) {
