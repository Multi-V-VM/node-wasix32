 ...
 public:
  ...
  void emitNotificationFromBackend(v8::Local<v8::Context> context,
    ...
    if (network_inspector_->canEmit(domain_name)) {
      ...
      UNREACHABLE(); // Unknown domain for emitNotificationFromBackend  // NOLINT(whitespace/comments, whitespace/line_length)
    }
  }
...
void Agent::ToggleNetworkTracking(Isolate* isolate, Local<Function> fn) {
  ...
  if (try_catch.HasCaught() && !try_catch.HasTerminated()) {
    ...
    UNREACHABLE(); // Cannot toggle network tracking, please report this.  // NOLINT(whitespace/comments, whitespace/line_length)
  }
}
...
void Agent::ToggleAsyncHook(Isolate* isolate, Local<Function> fn) {
  ...
  if (try_catch.HasCaught() && !try_catch.HasTerminated()) {
    PrintCaughtException(isolate, context, try_catch);
    UNREACHABLE(); // Cannot toggle Inspector's AsyncHook, please report this.  // NOLINT(whitespace/comments, whitespace/line_length)
  }
}
